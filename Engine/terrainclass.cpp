...
bool TerrainClass::InitializeBuffers(ID3D11Device* device)
{
	...
	for (j = xOffset; j<(m_terrainHeight - 1 + xOffset); j++)
	{
		for (i = zOffset; i<(m_terrainWidth - 1 + zOffset); i++)
		{
			// LINE 1
			// Upper left.
			positionX = (float)i;
			...
			vertices[index].position = D3DXVECTOR3(positionX, yOffset, positionZ);
